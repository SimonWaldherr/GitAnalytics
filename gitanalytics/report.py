...
function renderOverview(){
  ...
  `<div class="grid kpis">${kpi('Repositories',num(s.repositories),`${num(s.active_repositories)} aktiv · ${num(s.dormant_repositories)} inaktiv`)}${kpi('Aktive Abdeckung',pct(s.active_repository_share),'Repositories mit aktueller Historie')}${kpi('Commits',num(s.commits),'für gefilterte Repositories')}${kpi('Neue Beitragende',num(s.new_authors_last_30_days),'erstmals aktiv in den letzten 30 Tagen')}${kpi('CI-Abdeckung',pct(s.ci_coverage),`${num(s.repositories_with_ci)} Repositories mit erkanntem CI`)}${kpi('Lizenz-Abdeckung',pct(s.license_coverage),`${num(s.repositories_with_license)} Repositories mit Lizenzhinweis`)}${kpi('Code-Churn',num(s.churn),`+${num(s.additions)} / −${num(s.deletions)}`)}${kpi('Kommentar-Dichte',pct(s.comment_density),`${num(s.comment_lines)} Kommentarzeilen`)}${kpi('Codezeilen',num(s.code_lines),'HEAD, ohne Leerzeilen')}${kpi('HEAD-Dateien',num(s.tree_files),bytes(s.tree_bytes))}</div>`+
