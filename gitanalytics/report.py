...
<div class="shell">
  ...
  <main class="main">
    ...
    <div class="content">
      <div class="filters" aria-label="Dashboard-Filter"><label>Zuletzt bearbeitet <select id="recent-filter"><option value="">Beliebiger Zeitpunkt</option><option value="1">letzter Tag</option><option value="7">letzte 7 Tage</option><option value="30">letzte 30 Tage</option><option value="180">letzte 180 Tage</option><option value="365">letzte 365 Tage</option><option value="730">letzte 2 Jahre</option></select></label><label>Repository <select id="repo-filter" multiple size="1"></select></label><label>Programmiersprache <select id="language-filter" multiple size="1"></select></label><label>Dateityp <select id="file-type-filter" multiple size="1"></select></label><button class="action" id="clear-filters" type="button">Filter zurücksetzen</button><span class="muted" id="filter-result" aria-live="polite" aria-atomic="true"></span></div>
...
function calendar(rows){
  ...
  const max=Math.max(...rows.map(r=>Number(r.commits||0)),1);
  const cells=rows.map(r=>{const attrs=(r.repository_names||[]).length?crossFilterAttrs('repositorySet','',r.repository_names):crossFilterAttrs('recent',daysAgo(r.date));return `<button type="button" class="calendar-cell"${attrs} style="background:var(--heat-${heatLevel(Number(r.commits),max)})" title="${date(r.date)}: ${num(r.commits)} Commits. Klick filtert Repositories."></button>`;}).join('');
  return `<div class="calendar" aria-label="Commit-Kalender">${cells}</div>${legend()}`;
}
...
const filters={repositories:new Set(),languages:new Set(),fileTypes:new Set(),recentDays:''};
...
function filteredRepositories(){return D.repositories.filter(r=>selectedMatch(filters.repositories,r.name)&&(!filters.languages.size||(r.languages||[]).some(l=>filters.languages.has(l.language)))&&(!filters.fileTypes.size||(r.file_types||[]).some(t=>filters.fileTypes.has(t.file_type)))&&recentMatch(r));}
...
function setupFilters(){
  const repo=document.getElementById('repo-filter'), language=document.getElementById('language-filter'), type=document.getElementById('file-type-filter'), recent=document.getElementById('recent-filter');
  ...
  const update=()=>{filters.repositories=selectedValues(repo);filters.languages=selectedValues(language);filters.fileTypes=selectedValues(type);filters.recentDays=recent.value;refreshDashboard();};
  repo.addEventListener('change',update); language.addEventListener('change',update); type.addEventListener('change',update); recent.addEventListener('change',update); document.getElementById('clear-filters').addEventListener('click',()=>{filters.repositories.clear();filters.languages.clear();filters.fileTypes.clear();filters.recentDays='';syncSelect(repo,filters.repositories);syncSelect(language,filters.languages);syncSelect(type,filters.fileTypes);recent.value='';refreshDashboard();}); refreshDashboard();
}
...
function renderOverview(){
  ...
  `<div class="grid grid-2 mt"><div class="card"><h3>Letzte 365 Tage</h3><div class="sub">Jeder Block entspricht einem Kalendertag</div>${calendar(a.calendar_365)}</div><div class="card"><h3>Datenhinweise</h3><div class="sub">Qualität und Reichweite der Auswertung</div>${q.warnings.length?q.warnings.map(w=>`<div class="notice">${esc(w)}</div>`).join(''):'<div class="notice good">Keine allgemeinen Datenwarnungen.</div>'}</div></div>`;
}
...
function renderRepositories(){
  ...
  const cols=[
    ...
    {key:'last_commit',label:'Letzter Commit',format:date},{key:'local_branches',label:'Branches',num:true,format:num},
